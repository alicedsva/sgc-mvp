...
export function MatrizProgressao({ cargos, habilidadesCargo }: MatrizProgressaoProps) {
  ...
  const getNivelColor = (nivel: string | null) => {
    if (!nivel) return 'bg-gray-50 text-gray-400';
    
    const colors: Record<string, string> = {
      'Básico': 'bg-blue-400 text-white',
      'Intermediário': 'bg-blue-600 text-white',
      'Avançado': 'bg-indigo-700 text-white',
      'Especialista': 'bg-violet-800 text-white',
    };
    ...
    <div>
      ...
      <div className="mb-4 flex items-center gap-6 text-xs text-gray-600">
        ...
        <div className="flex items-center gap-1">
          <span className="inline-flex px-2 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-800">Bás</span>
          <span>Básico</span>
        </div>
        <div className="flex items-center gap-1">
          <span className="inline-flex px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800">Int</span>
          <span>Intermediário</span>
        </div>
        <div className="flex items-center gap-1">
          <span className="inline-flex px-2 py-1 text-xs font-medium rounded-full bg-indigo-100 text-indigo-800">Ava</span>
          <span>Avançado</span>
        </div>
        <div className="flex items-center gap-1">
          <span className="inline-flex px-2 py-1 text-xs font-medium rounded-full bg-purple-100 text-purple-800">Esp</span>
      ...
      <div className="mb-6 flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white focus:ring-2 focus:ring-[var(--brand-500)] focus:border-[var(--brand-500)] outline-none"
          >
                    ...
                    {cargos.map(cargo => {
                      ...
                      return (
                        <td key={cargo.id} className="px-4 py-3 text-center">
                          <span
                            className={`inline-flex items-center justify-center px-2.5 py-1 text-xs font-medium rounded-full min-w-[48px] ${getNivelColor(nivel)}`}
